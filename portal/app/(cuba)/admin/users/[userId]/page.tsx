...
export default function UserDetailPage() {
  ...
  const fetchUser = async () => {
    try {
      ...
      setNewTier(data.user.organization?.tier || 'FREE');
    } catch (err) {
  ...
  const handleUpdateTier = async () => {
    if (!user || newTier === (user.organization?.tier || 'FREE')) {
  ...
  if (error) {
    return (
      <div className="bg-[#FC564A]/10 border border-[#FC564A]/20 rounded-xl p-6">
        <p className="text-[#FC564A] mb-4">{error}</p>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-mono text-[#7366FF]">User ID: {user.userId}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            {user.activeSessions > 0 && (
              <button
                ...
                className="px-4 py-2 border border-[#FC564A]/30 text-[#FC564A] rounded-lg font-medium hover:bg-[#FC564A]/10 transition-colors"
              >
                Force Logout
              </button>
            )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                {editingRole ? (
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      user.role === 'SUPER_ADMIN' ? 'bg-[#FC564A]/10 text-[#FC564A]' :
                      user.role === 'ADMIN' ? 'bg-purple-500/10 text-purple-400' :
                      'bg-white/5 text-[#A8A9AD]'
                    }`}>
                      {user.role.replace('_', ' ')}
                    </span>
                    <button
                  ...
                  {editingTier ? (
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      >
                        ...
                        <option value="DEVELOPER">Starter ($29/mo)</option>
                        <option value="TEAM">Professional ($49/mo)</option>
                        <option value="ENTERPRISE">Enterprise</option>
                      </select>
                      <button
                        onClick={handleUpdateTier}
                        disabled={tierUpdating}
                        className="px-3 py-1 bg-[#7366FF] text-white rounded-lg text-sm font-medium hover:bg-[#5A4FCF] disabled:opacity-50"
                      >
                        {tierUpdating ? '...' : 'Save'}
                      </button>
                      <button
                        onClick={() => {
                          setEditingTier(false);
                          setNewTier(user.organization?.tier || 'FREE');
                        }}
                        className="px-3 py-1 border border-white/10 rounded-lg text-sm"
                      >
                        Cancel
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${
                        user.organization.tier === 'ENTERPRISE' ? 'bg-[#FFB829]/10 text-[#FFB829]' :
                        user.organization.tier === 'TEAM' ? 'bg-[#40B8F4]/10 text-[#40B8F4]' :
                        user.organization.tier === 'DEVELOPER' ? 'bg-green-500/10 text-green-400' :
                        'bg-white/5 text-[#A8A9AD]'
                      }`}>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {user.organization.sorobanProEnabled && (
                      <span className="px-2 py-0.5 bg-purple-500/10 text-purple-400 rounded text-xs">Soroban Pro</span>
                    )}
                    {user.organization.intelligenceEnabled && (
                      <span className="px-2 py-0.5 bg-[#40B8F4]/10 text-[#40B8F4] rounded text-xs">Intelligence</span>
                    )}
                    {user.organization.portfolioEnabled && (
                      <span className="px-2 py-0.5 bg-green-500/10 text-green-400 rounded text-xs">Portfolio</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          {!user.organization && user.role !== 'SUPER_ADMIN' && (
            <div className="bg-[#262932] rounded-xl border border-white/10 p-6">
              <h2 className="text-lg font-bold text-white mb-4">Subscription</h2>
              <p className="text-sm text-[#A8A9AD] mb-3">This user has no subscription. Assign a tier:</p>
              <div className="flex items-center gap-2">
                <select
                  value={newTier}
                  onChange={(e) => setNewTier(e.target.value)}
                  className="px-3 py-2 border border-white/10 rounded-lg focus:outline-none focus:border-[#7366FF] bg-[#1D1E26] text-sm"
                >
                  <option value="FREE">Free</option>
                  <option value="DEVELOPER">Starter ($29/mo)</option>
                  <option value="TEAM">Professional ($49/mo)</option>
                ...
                <button
                  onClick={handleUpdateTier}
                  disabled={tierUpdating}
                  className="px-4 py-2 bg-[#7366FF] text-white rounded-lg text-sm font-medium hover:bg-[#5A4FCF] disabled:opacity-50"
                >
                  {tierUpdating ? 'Assigning...' : 'Assign Tier'}
                </button>
              </div>
            </div>
          )}
                ...
                {user.apiKeys.map((key) => (
                  <div key={key.id} className="p-3 border border-white/10 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {key.revokedAt ? (
                        <span className="px-2 py-0.5 bg-[#FC564A]/10 text-[#FC564A] rounded text-xs">Revoked</span>
                      ) : (
                        <span className="px-2 py-0.5 bg-green-500/10 text-green-400 rounded text-xs">Active</span>
                      )}
                    </div>
